...
// ===== 启动前馈（open-loop boot） =====
float U_BOOT = 60.0f;                     // 启动占空比 (0..255)
...
void serial_cli() {
  ...
  if (low.startsWith("set target")) {
    ...
    Serial.println(F("Unknown cmd. Use: 'set target <v>', 'set kp <v>', 'set ki <v>', 'set bootu <v>', 'set boott <ms>', 'status'"));
  }
}
...
void setup() {
  ...
  Serial.println(F("PI controller ready (boot->closed loop). Commands: set target / set kp / set ki / set bootu / set boott / status"));
...
void control_step() {
  ...
  if (in_boot) {
    if ((long)(now - boot_until) < 0) {
      ...
      // 启动期结束：切闭环，清积分，窗口保留（已平滑起来）
      in_boot = false;
      integ_e = 0.0f;
      // 可选：如需更“干净”，也可清窗口
      // win_idx=0; win_count=0; for (uint8_t i=0;i<N_WIN;i++) win_buf[i]=0;
    }
  }
